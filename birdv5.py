...
async def read_channel_messages_by_id(channel_id, limit=5):
...
def format_results(results):
    ...
    output = "**Best signals & statistics:**\n\n"
    ...
    # Convert the time_to_highest to a more readable format
        ...
        time_to_highest_readable = f"{hours} hour{'s' if hours != 1 else ''} {minutes} minute{'s' if minutes != 1 else ''}"

        output += f"**# {row['token_name']} - {row['percentage_change']:.2f}%**\n"
    ...
    output += f"**Total Positive Percentage Changes:** {total_positive_percentage}\n"
    output += f"**Total Zero or Negative Percentage Changes:** {total_zero_or_negative_percentage}\n"
